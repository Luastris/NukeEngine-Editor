...
TextEditor::Coordinates TextEditor::FindWordStart(const Coordinates& aFrom) const
{
	...
	bool initialIsSpace = isspace((unsigned char)line[charIndex].mChar);   // NukeEngine: UTF-8-safe
	...
	while (Move(lineIndex, charIndex, true, true))
	{
		...
		bool isSpace = isspace((unsigned char)line[charIndex].mChar);   // NukeEngine: UTF-8-safe
				...
				if (token_color == PaletteIndex::Identifier)
				{
					...
					if (!mLanguageDefinition->mCaseSensitive)
						std::transform(id.begin(), id.end(), id.begin(),
							[](char ch) { return (char)::toupper((unsigned char)ch); });   // NukeEngine: UTF-8-safe
	...
	if (mCheckComments)
	{
		...
		while (currentLine < endLine || currentIndex < endIndex)
		{
			...
			if (!line.empty())
			{
				...
				if (c != mLanguageDefinition->mPreprocChar && !isspace((unsigned char)c))   // NukeEngine: UTF-8-safe
					firstChar = false;
